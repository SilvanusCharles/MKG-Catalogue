  ...
  head: () => ({
    meta: [
      { title: "Products — MKG Kabel Catalogue" },
      {
        name: "description",
        content:
          "Browse the full MKG Kabel catalogue — armoured cable, PVC/PVC, flexible cable, conduit, pipes and earthing materials.",
      },
    ],
  }),
...
function ProductsPage() {
  ...
  const setCat = (c: Category | undefined) => navigate({ search: c ? { category: c } : {} });
    ...
    <>
      <section className="bg-[color:var(--brand-black)] text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
          ...
          <h1 className="mt-2 font-display font-black text-4xl md:text-5xl uppercase">
            Our Products
          </h1>
          <p className="mt-3 text-white/70 max-w-2xl text-sm md:text-base">
            {isLoading ? "Loading…" : `${filtered.length} products available.`} Contact us for bulk
      ...
      <section className="bg-white border-b border-border sticky top-16 z-30">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3 flex gap-2 overflow-x-auto">
          <FilterPill active={!category} onClick={() => setCat(undefined)}>
            All
          </FilterPill>
          {CATEGORIES.map((c) => (
